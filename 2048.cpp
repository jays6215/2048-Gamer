...
bool Grid::shift(dir_e dir, int mode){
    int nTile;
    int lastMerge;
    ...
    for(int i = 0;i < GRID_LENGTH;i++){
        ...
        for(int j = 0;j < GRID_LENGTH;j++){
            if((this->*getDirEntry)(i, j) == EMPTY)
                continue;
            if( mode == 1 && (this->*getDirEntry)(i, j) > 0 )
                cnt_tiles++ ;
            if(nTile > lastMerge && (this->*getDirEntry)(i, j) == shiftBuff[nTile - 1]){
                ...
                isShifted = TRUE;
                if( mode == 1 )
                    m_score += shiftBuff[nTile - 1], m_nEmptyBlk++;
                add_score += shiftBuff[nTile-1], empty_blk += 1;
            }
            else{
        ...
        for(int j = 0;j < GRID_LENGTH;j++)
        {
            ...
            if( mode == 1 )
                ...
                Tsa[(int)dir][rot/4][rot%4] = shiftBuff[j];
            sum_tiles += shiftBuff[j] ;
            if( mode == 0 && shiftBuff[j] > 0 )
                cnt_tiles++ ;
        }
    }
    if( mode == 0 )
    {
        Tsa[(int)dir][4][0] = add_score ;
        Tsa[(int)dir][4][1] = empty_blk ;
        Tsa[(int)dir][4][2] = sum_tiles ;
        Tsa[(int)dir][4][3] = cnt_tiles ;
    }
    else
    {
        Tsa[4][4][0] = 0 ;
